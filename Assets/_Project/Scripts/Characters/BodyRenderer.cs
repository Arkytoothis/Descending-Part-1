...
namespace Descending.Characters
{
    ...
    public class BodyRenderer : MonoBehaviour
    {
        ...
        {
            if (item == null || item.GetWeaponData() == null) return;

            if (item.ItemDefinition.EquipmentSlot == EquipmentSlot.Right_Hand)
            {
            ...
            else if (item.ItemDefinition.EquipmentSlot == EquipmentSlot.Left_Hand)
            {
