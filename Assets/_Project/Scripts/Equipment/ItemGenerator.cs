...
namespace Descending.Equipment
{
    public static class ItemGenerator
    {
        ...
        public static Item GenerateRandomItem(RarityDefinition bestMaterial, EquipmentSlot slot, int plusChance, int prefixChance, int suffixChance)
        {
            string itemKey = _itemsBySlot[(int)slot][UnityEngine.Random.Range(0, _itemsBySlot[(int)slot].Count)];
            Item item = new Item(Database.instance.Items.GetItem(itemKey));
            MaterialDefinition material = GetItemMaterial(item, bestMaterial);
            item.SetMaterial(material);
            RandomizeQuality(ref item, plusChance);
        ...
        public static ItemShort GenerateRandomItemShort(RarityDefinition bestMaterial, int bestQuality, int plusChance, int prefixChance, int suffixChance)
        {
            ...
            {
                quality = _qualityEnchants[UnityEngine.Random.Range(5, _qualityEnchants.Count)].Key;
            }
            else if (bestQuality > -1)
            {
                quality = _qualityEnchants[UnityEngine.Random.Range(0, bestQuality)].Key;
            }

            if (UnityEngine.Random.Range(0, 100) < prefixChance)
            {
                prefix = _prefixEnchants[UnityEngine.Random.Range(0, _prefixEnchants.Count)].Key;
            }

            if (UnityEngine.Random.Range(0, 100) < suffixChance)
            {
                suffix = _suffixEnchants[UnityEngine.Random.Range(0, _suffixEnchants.Count)].Key;
            }

            ItemShort itemShort = new ItemShort(item.Key, quality, item.MaterialKey, prefix, suffix);
        ...
        {
            if (Database.instance.Items.Contains(itemShort.ItemKey) == false)
            {
                Debug.Log("Key: " + itemShort.ItemKey + " does not exist");
                return null;
            }

            Item item = new Item(Database.instance.Items.GetItem(itemShort.ItemKey));
            MaterialDefinition material = GetItemMaterial(item, bestMaterial);
            item.SetMaterial(material);
            item.CalculateValue();
        ...
        public static Item GenerateItem(ItemShort itemShort)
        {
            ...
            Item item = new Item(Database.instance.Items.GetItem(itemShort.ItemKey));
            MaterialDefinition material = Database.instance.Materials.GetMaterial(itemShort.MaterialKey);
            item.SetMaterial(material);
            //EnchantmentDefinition quality = Database.instance.Enchants.GetEnchant(itemShort.QualityKey);
            //item.SetQualityEnchant(new Enchantment(quality));
            item.CalculateValue();
        ...
        private static void LoadItemsBySlot()
        {
            ...
            foreach (var item in Database.instance.Items.Dictionary)
            {
                if (item.Value.EquipmentSlot != EquipmentSlot.None)
                {
